...
from utils.localizers import get_counts_from_bm_eval

from .bipartite_match import bipartite_match
from utils.base import remove_border_points_from_df, evaluate_df
...
class SpatialMeanShift:
    ...
    def get_seeds(self, x, radius=1, threshold="auto"):
        ...
        maxima = x == sp_img.maximum_filter(
            x, size=k_size.astype(int), mode="constant", cval=0.0
        )
        ...
        if self.exclude_border:
            x, y, z = x.shape
            seeds = seeds[
                (seeds[:, 0] >= self.exclude_border[0] // 2)
                & (seeds[:, 0] < x - self.exclude_border[0] // 2)
            ]
            seeds = seeds[
                (seeds[:, 1] >= self.exclude_border[1] // 2)
                & (seeds[:, 1] < y - self.exclude_border[1] // 2)
            ]
            seeds = seeds[
                (seeds[:, 2] >= self.exclude_border[2] // 2)
                & (seeds[:, 2] < z - self.exclude_border[2] // 2)
            ]
        return seeds
    ...
    def _remove_duplicates(self, center_mass_dict, radius):
        ...
        sorted_by_intensity = sorted(
            center_mass_dict.items(), key=lambda tup: tup[1], reverse=True
        )
    ...
    def predict(
        ...
        RET = joblib.Parallel(n_jobs=n_jobs, prefer="threads")(
            joblib.delayed(climb)(m) for m in seeds
        )
    ...
    def predict_and_evaluate(
        ...
        """
        y_pred = self.predict(
            x, kernel_radius, peaks_dist, threshold, seeds, n_jobs, max_iterations
        )
