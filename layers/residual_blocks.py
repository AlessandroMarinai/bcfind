...
class ResidualDecoderBlock(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        if self.upsample and self.skip_connection_in:
            if self.normalization == "batch":
        ...
        if not self.skip_connection_in:
            if self.normalization == "batch":
                self.up_norm_no_concat = tf.keras.layers.BatchNormalization()
            elif self.normalization == "switch":
                self.up_norm_no_concat = SwitchNormalization()
            elif self.normalization == "instance":
                self.up_norm_no_concat == tf.keras.layers.UnitNormalization()
            elif self.normalization == "layer":
                self.up_norm_no_concat = tf.keras.layers.LayerNormalization()

            self.up_activ_no_concat = tf.keras.layers.Activation(self.activation)
            self.conv3D_T_no_concat = tf.keras.layers.Conv3DTranspose(
    ...
    def build(self, input_shape):
        ...
        elif self.upsample and not self.skip_connection_in:
            self.up_norm_no_concat.build(input_shape)
            self.up_activ_no_concat.build(input_shape)
            self.conv3D_T_no_concat.build(input_shape)
            out_shape_1 = self.conv3D_T_no_concat.compute_output_shape(input_shape)
        else:
            out_shape_1 = input_shape
    ...
    def call(self, inputs, to_concatenate_layer=None, training=None):
        if self.upsample and to_concatenate_layer is not None:
            h = self.up_norm(inputs)
            h = self.up_activ(h)
            h = self.conv3D_T(h)

        elif not self.skip_connection_in:
            h = self.up_norm_no_concat(inputs)
            h = self.up_activ_no_concat(h)
            h = self.conv3D_T_no_concat(h)
