...
from utils.data import get_input_tf, get_input_tf_slice
...
class TrainingDataset_DA_slice(tf.keras.utils.Sequence):
    ...
    @staticmethod
    @tf.function
    def parse_imgs_target_slice(array, preprocess_kwargs):
        input_image = get_input_tf_slice(array, **preprocess_kwargs)
        input_image = tf.expand_dims(array, 0)
        return tf.ensure_shape(input_image, (1, 50, 100, 100))
    ...
    def __new__(
        ...
        **preprocess_kwargs,
        ...
        if not use_lmdb_data:
            ...
            crops = []
            for i in range(20):
                crop = img_to_crop[0:output_shape[0],0:output_shape[1],0:output_shape[2]]
                print(i)
                print(crop.shape)
                crops.append(crop)
            ...
            target_domain_data = tf.data.Dataset.from_tensor_slices((crops))
            
            target_domain_data = target_domain_data.astype(tf.float32)
            print(target_domain_data)
            target_domain_data = target_domain_data.map(
                lambda x: cls.parse_imgs_target_slice(x, preprocess_kwargs),
                num_parallel_calls=tf.data.AUTOTUNE,
                deterministic=False,
            )
            # cache data after time consuming map and make it shuffle every epoch
            target_domain_data = target_domain_data.cache().shuffle(len(tiff_list), reshuffle_each_iteration=True)
            ...
            return super(TrainingDataset_DA_slice, cls).__new__(cls)
