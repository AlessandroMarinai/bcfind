...
from config_manager import TrainConfiguration
...
class Trainer:
    ...
    def run(
        ...
        if test_as_val:
            ...
            db_name = f"{conf.data.name}_val_pred_lmdb"
        else:
            tiff_files = train_tiff_files
            marker_files = train_marker_files
            db_name = f"{conf.data.name}_train_pred_lmdb"
    ...
    def test(self, config_file, test_as_val, gpu=-1):
        ...
        self.dog_test_inputs, self.dog_test_targets = self.make_dog_data(
            ...
            data_shape=conf.data.shape,
            lmdb_dir=f"{conf.exp.basepath}/{conf.data.name}_test_pred_lmdb",
            **conf.preproc,
        )
        cuda.close()
        ...
        res = pd.concat(res)
        res.to_csv(f"{conf.exp.basepath}/{conf.data.name}_test_eval.csv", index=False)
