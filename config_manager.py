...
class TrainDAConfiguration:
    def __init__(self, yaml_file):
        ...
        # Source dataset
        self.source.shape = np.array(self.source.shape)
        self.source.dim_resolution = np.array(self.source.dim_resolution)

        self.source.train_tif_dir = f"{self.source.basepath}/Tiff_files/Train"
        self.source.train_gt_dir = f"{self.source.basepath}/GT_files/Train"
        self.source.test_tif_dir = f"{self.source.basepath}/Tiff_files/Test"
        self.source.test_gt_dir = f"{self.source.basepath}/GT_files/Test"

        # Target dataset (train without ground truth)
        self.target.shape = np.array(self.target.shape)
        self.target.dim_resolution = np.array(self.target.dim_resolution)

        self.target.train_tif_dir = f"{self.target.basepath}/Tiff_files/Train"

        self.target.test_tif_dir = f"{self.target.basepath}/Tiff_files/Test"
        self.target.test_gt_dir = f"{self.target.basepath}/GT_files/Test"
        ...
        self.unet.checkpoint_dir = f"{self.exp.basepath}/UNet_checkpoints"
        self.unet.tensorboard_dir = f"{self.exp.basepath}/UNet_tensorboard"
        self.unet.exclude_border = self.source.cell_radius
        ...
        # DoG solo sulla source
        self.dog.exclude_border = np.array(self.source.cell_radius) * 2

        self.dog.checkpoint_dir = f"{self.exp.basepath}/DoG_checkpoints"
        self.dog.predictions_dir = f"{self.exp.basepath}/{self.source.name}_predictions"
