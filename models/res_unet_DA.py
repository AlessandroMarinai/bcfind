...
class ResUNet_DA(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super(ResUNet_DA, self).__init__(**kwargs)
    ...
    def call(self, inputs, training=None):
        ...
        for i_d, decoder_block in enumerate(self.decoder_blocks):
            if i_d == 0:
                ...
                h = decoder_block(h, h1, training=training)  #if you want to remove the skip set skip_connection_in parameter of this block to False

            if self.dropout:
                h = self.dropouts[i_e + 2 + i_d](h, training=training)
                
            if i_d == self.n_blocks - 1:
                h_to_pred = h

        # Predictor
        domain_pred = self.domain_classifier(h_to_pred)
    ...
    def get_config(
        ...
        config = super(ResUNet_DA, self).get_config()
        config.update(
            {
